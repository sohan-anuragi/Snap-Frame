...
import { rugItems } from "./HomeDecorData";

export default function HomeDecorRugs() {
  ...
  return (
    <>
      <li
        className={`
              ${rugsOpen === true ? "" : "hover:bg-gray-100"}
    relative px-2
    pl-6 py-3
  ...
  `}
      >
        <button
          type="button"
          onClick={() => setRugsOpen((prev) => !prev)}
          className="flex w-full items-center justify-between pr-4"
        >
          <span className="font-bold  text-[#000000cb]">Personalized Rugs</span>
